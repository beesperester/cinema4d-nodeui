...
class Node(metaclass=NodeType):
    ...
    def __init__(self, data_type: int, long_id: str, **kwargs: Any) -> None:
        ...
        if "apply" in kwargs.keys() and isinstance(kwargs["apply"], list):
            self.apply = kwargs["apply"]

        if "nodes" in kwargs.keys() and isinstance(kwargs["nodes"], list):
            self.nodes = kwargs["nodes"]

        if "dependencies" in kwargs.keys() and isinstance(
    ...
    def desc_id(self) -> c4d.DescID:
        ...
        """
        return c4d.DescID(c4d.DescLevel(self.id, self.data_type))
    ...
    def Register(
        ...
        if single_id is None or self.desc_id.IsPartOf(single_id)[0]:
            result.append(
                description.SetParameter(self.desc_id, node, group_id)
            )

        if self.Get(data_instance) is None and self.default is not None:
            self.Set(data_instance, self.default)

        for node in self.nodes:
            result.append(
                node.Register(
                    self.desc_id, description, data_instance, single_id
                )
            )

        return False not in result

    def Get(self, data_instance: c4d.BaseContainer) -> Any:
        return data_instance[self.id]

    def Set(self, data_instance: c4d.BaseContainer, value: Any) -> bool:
